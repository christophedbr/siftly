...
async function getAllCategories() {
  ...
  _categoriesCache = await prisma.category.findMany({
...
async function detectIntentCategories(query: string): Promise<string[]> {
  ...
  const INTENT_SIGNALS: Array<{ pattern: RegExp; slugHints: string[] }> = [
    {
      pattern:
        /meme|funny|humor|laugh|lol|joke|hilarious|comedy|satire|roast|viral|dank|wholesome|cringe|based|skit|parody/i,
      slugHints: ["funny-memes"],
    },
    {
      pattern:
        /crypto|bitcoin|btc|eth|sol|defi|nft|trading|chart|invest|token|wallet|blockchain|altcoin|memecoin|pump|dump|yield|staking|airdrop|web3|dex|liquidity|portfolio|stocks|options|macro|economy|dollar|inflation|fed|interest rate/i,
      slugHints: ["finance-crypto"],
    },
    {
      pattern:
        /code|coding|developer|programming|github|software|api|framework|library|debug|deploy|devops|backend|frontend|fullstack|typescript|javascript|python|rust|golang|java|swift|css|html|sql|cli|terminal|bash|docker|kubernetes|ci.?cd|open.?source|repo|pull request|lint/i,
      slugHints: ["dev-tools"],
    },
    {
      pattern:
        /\bai\b|llm|gpt|claude|openai|anthropic|gemini|mistral|llama|model|prompt|agent|machine learning|neural|deep learning|embedding|rag|fine.?tun|inference|transformer|diffusion|multimodal|copilot|cursor|midjourney|stable diffusion|image gen/i,
      slugHints: ["ai-resources"],
    },
    {
      pattern:
        /design|ui\b|ux\b|figma|typography|branding|logo|visual|color palette|layout|wireframe|prototype|tailwind|animation|motion|font|icon|component|design system|saas ui/i,
      slugHints: ["design"],
    },
    {
      pattern:
        /productivity|focus|habit|workflow|time management|system|note.?taking|task|pkm|second brain|obsidian|notion|roam|logseq|journaling|calendar|routine|morning|discipline|deep work|async/i,
      slugHints: ["productivity"],
    },
    {
      pattern:
        /news|breaking|announcement|launch|update|thread|essay|opinion|analysis|report|geopolitic|politic|regulation|lawsuit|acquisition|ipo|funding|raised|series/i,
      slugHints: ["news"],
    },
  ];
      ...
      for (const hint of slugHints) {
        for (const cat of allCats) {
          if (
            ...
            (cat.description &&
              cat.description.toLowerCase().includes(hint.split("-")[0]))
          ) {
            slugs.add(cat.slug);
          }
        }
...
function buildIndexEntry(b: {
  ...
  for (const m of b.mediaItems) {
    ...
    try {
      ...
      if (Array.isArray(p.text_ocr) && (p.text_ocr as string[]).length)
        parts.push(
          `ocr="${(p.text_ocr as string[]).join(" | ").slice(0, 250)}"`,
        );
  ...
  if (b.entities) {
    try {
      ...
      const parts: string[] = [];
      if (ent.hashtags?.length)
        parts.push(`#${ent.hashtags.slice(0, 10).join(" #")}`);
      if (ent.tools?.length) parts.push(`tools: ${ent.tools.join(", ")}`);
      if (ent.mentions?.length)
        parts.push(`mentions: @${ent.mentions.slice(0, 5).join(" @")}`);
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const [keywordHits, intentHits] = await Promise.all([
    keywords.length > 0
      ? prisma.bookmark.findMany({
          where: {
            ...categoryFilter,
            ...(useFts ? { id: { in: ftsIds } } : { OR: keywordConditions }),
          },
          // FTS results are pre-ranked; LIKE results fall back to recency ordering
          orderBy: useFts
            ? undefined
            : [{ enrichedAt: "desc" }, { tweetCreatedAt: "desc" }],
          take: MAX_CANDIDATES,
          select: selectShape,
        })
      : prisma.bookmark.findMany({
  ...
  if (bookmarks.length < 20) {
    ...
    for (const b of fallback) {
      if (!fallbackSeen.has(b.id)) {
        fallbackSeen.add(b.id);
        bookmarks.push(b);
      }
    }
    // Cap total
  ...
  if (bookmarks.length === 0) {
    return NextResponse.json({
      bookmarks: [],
      explanation: "No bookmarks found.",
    });
  }
...
- reason must be specific, not generic ("shows bitcoin price crash chart" not "related to crypto")`;
  ...
  try {
    const rawText = await chatComplete(prompt, { maxTokens: 1500 });
