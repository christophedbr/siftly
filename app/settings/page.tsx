    ...
    >
      <div className="flex items-start gap-3 mb-5">
        ...
        >
          <Icon
            size={16}
            className={isDanger ? "text-red-500" : "text-indigo-400"}
          />
        </div>
        <div>
          <h2
            className={`text-base font-semibold ${isDanger ? "text-red-400" : "text-zinc-100"}`}
          >
            {title}
          </h2>
...
function ApiKeyField({
  ...
  hasKeyField: "hasAnthropicKey" | "hasOpenaiKey";
  hint: string;
  docHref: string;
  ...
  async function handleSave() {
    ...
    try {
      ...
      setSavedMasked(
        key.trim().slice(0, 6) + "••••••••" + key.trim().slice(-4),
      );
      setKey("");
        ...
        <div className="flex items-center gap-2 min-w-0 overflow-hidden">
          {savedMasked && (
            <span className="flex items-center gap-1.5 text-xs text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-lg min-w-0 overflow-hidden">
              ...
              <span className="font-mono truncate">{savedMasked}</span>
            </span>
          )}
          {savedMasked && (
            <button
      ...
      <div className="flex gap-2.5">
        <div className="relative flex-1">
          <input
            ...
            placeholder={
              savedMasked ? "Enter new key to replace…" : placeholder
            }
...
function ModelSelector({
  ...
  useEffect(() => {
    fetch("/api/settings")
      .then((r) => r.json())
      .then((d) => {
        if (d[settingKey]) setValue(d[settingKey] as string);
      })
      .catch(() => {});
  }, [settingKey]);
    ...
    <>
      ...
      {value === "claude-opus-4-6" && (
        <p className="text-xs text-amber-500/80 mt-1.5">
          Opus is slow with 20 parallel workers — consider Sonnet or Haiku for
          faster bulk categorization.
        </p>
      )}
    </>
  );
      ...
      <div className="flex gap-3 p-3.5 rounded-xl bg-emerald-500/5 border border-emerald-500/20 mb-5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-zinc-500 mt-0.5 leading-relaxed">
            Signed in as <span className="text-zinc-300">{tier}</span> via
            Claude Code. Siftly will use your subscription automatically. An API
            key below will take priority if set.
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex gap-3 p-3.5 rounded-xl bg-amber-500/5 border border-amber-500/20 mb-5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-zinc-500 mt-0.5">
            Run <span className="font-mono text-zinc-300">claude</span> in your
            terminal to refresh the session, then reload this page.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex gap-3 p-3.5 rounded-xl bg-zinc-800/60 border border-zinc-700 mb-5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-zinc-500 mt-0.5 leading-relaxed">
          Install Claude Code and sign in to skip the API key entirely, or paste
          your API key below.
        </p>
      </div>
    </div>
...
function ProviderToggle({ onToast }: { onToast: (t: Toast) => void }) {
  ...
  useEffect(() => {
    fetch("/api/settings")
      .then((r) => r.json())
      .then((d) => {
        if (d.aiProvider) setProvider(d.aiProvider as "anthropic" | "openai");
      })
      .catch(() => {});
  }, []);

  async function handleChange(p: "anthropic" | "openai") {
    setProvider(p);
    try {
      const res = await fetch("/api/settings", {
        method: "POST",
      ...
      onToast({ type: "error", message: "Failed to save provider" });
    }
  }
    ...
    <div className="mb-5">
      ...
      {saved && (
        <p className="text-xs text-emerald-400 mt-1.5 flex items-center gap-1">
          <Check size={11} /> Provider updated
        </p>
      )}
    </div>
  );
...
function ApiKeySection({ onToast }: { onToast: (t: Toast) => void }) {
  return (
    <Section
      icon={Key}
      title="AI Provider"
      description="Choose between Anthropic or OpenAI for categorization and search."
    >
      <ProviderToggle onToast={onToast} />
      ...
      <div className="space-y-5">
        ...
        <div>
          <ApiKeyField
            ...
            hint="Used when OpenAI is the active provider."
        ...
        <div>
          <ApiKeyField
            ...
            hint="Used when Anthropic is the active provider."
      ...
      <p className="text-xs text-zinc-600 mt-4">
        Keys are stored encrypted in the database. Only the active provider is
...
function AboutSection() {
  ...
  return (
    <Section
      icon={Info}
      title="About Siftly"
      description="Self-hosted Twitter bookmark manager"
    >
      <p className="text-sm text-zinc-400 leading-relaxed mb-5">
        <strong className="text-zinc-100 font-semibold">Siftly</strong> is a
        self-hosted app for organizing your Twitter/X bookmarks. Use the
        built-in bookmarklet or console script to import, then run the 4-stage
        AI pipeline to analyze images, extract entities, generate semantic tags,
        and auto-categorize — then explore connections through the interactive
        mindmap.
      </p>
        ...
        >
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-zinc-600">
              Built &amp; open-sourced by
            </p>
          </div>
          <ExternalLink
            size={12}
            className="text-zinc-600 group-hover:text-zinc-400 transition-colors ml-auto shrink-0"
          />
        </a>
    ...
    <div className="p-6 md:p-8 max-w-2xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-zinc-400 mt-1 text-sm">
          Configure your Siftly instance
        </p>
      </div>
