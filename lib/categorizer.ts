...
function buildCategorizationPrompt(
  ...
  const tweetData = bookmarks.map((b) => {
    ...
    if (b.semanticTags?.length)
      entry.aiTags = b.semanticTags.slice(0, 20).join(", ");
...
export async function categorizeBatch(
  ...
  // Check if using OpenAI provider — skip CLI attempts
  const provider = await getAIProvider();
  if (provider === "openai") {
    const text = await chatComplete(prompt, { maxTokens: 2048 });
    return parseCategorizationResponse(text, new Set(allSlugs));
  }

  // Anthropic: prefer CLI over SDK (avoids OAuth token extraction)
  if (await getCliAvailability()) {
    ...
    if (result.success && result.data) {
      try {
        ...
        console.warn(
          "[categorize] CLI response parse failed, falling back to SDK:",
          parseErr,
        );
      }
    } else {
      console.warn(
        "[categorize] CLI failed, falling back to SDK:",
        result.error,
      );
    }
  }
  ...
  if (!client) {
    throw new Error(
      "No AI provider configured. Go to Settings to add an API key.",
    );
  }
...
export function mapBookmarkForCategorization(b: {
  ...
  if (b.semanticTags) {
    try {
      semanticTags = JSON.parse(b.semanticTags) as string[];
    } catch {
      /* ignore */
    }
  }
